...
- **Using selection input to avoid foreign key reference error:** I chose to use selection as the input type when the input item is foreign key referencing another table such as the team in adding members, event in adding event stages, stage and member in adding scores. This could effectively avoid the issue that user input a data that does not exist in the referenced table.
...
- **Input format assumption:** I assumed that the name of events, stages, members, sports, city, location could be the combination of any letters, numbers or symbols, so they don't have to be only letters and there's no validtion check for it. Except for two inputs - points scored and points to qualify - have to be numbers and these two involves validation check achieved by Javascript and Bootstrap.
- **Using Javascript together with Bootstrap to validate the input:** I found it's tricky to validate the input from the server side and then return feedback. I was expecting to validate some input before it's been passed to the server. It seems that the only solution is through Javascript. After some searching I found a piece of useful Javascript code below which could do this job very well. I put it into the base page *admin.html* so it could help validate all the inputs under the admin interface. It can both validate if the require field is empty and if the input is number at Pointstoqualify and Score field.
```Javascript
            <script>
